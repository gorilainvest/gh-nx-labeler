...
function getEnvironmentVariables(): EnvironmentVaribles {
  ...
  const allAffectedTag =
    process.env.ALL_AFFECTED_TAG ?? 'all projects affected';
  ...
  const labelPrefix = process.env.LABEL_PREFIX_DEFINITIONS
    ? (JSON.parse(process.env.LABEL_PREFIX_DEFINITIONS) as Record<
        string,
        LabelPrefixDefinition
      >)
    : defaultLabelPrefixDefinitions;

  const projectTypeAbbreviations = process.env.PROJECT_TYPE_ABBREVIATIONS
    ? (JSON.parse(process.env.PROJECT_TYPE_ABBREVIATIONS) as Record<
        string,
        string
      >)
...
async function collectAffectedTags(
  ...
  if (affected.length === Object.keys(configurations).length) {
    ...
    for (const project of affected) {
      ...
      tags.add(
        `${projectTypeAbbreviations[config.projectType]}:${config.name}`
      );
    }
  }
  return tags;
...
export async function run(): Promise<void> {
  const { token, allAffectedTag, projectTypeAbbreviations, labelPrefix } =
